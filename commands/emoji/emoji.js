...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("emoji")
    .setDescription(
      "Run this command with an attached image suited to be an emoji. Moderators will approve the emoji.",
    )
  ...
  async execute(interaction) {
    ...
    if (!emoji_name.match(/^[\w\d_]{2,}$/)) {
      interaction.reply(
        `Improper name ${emoji_name} provided! Name must be at least 2 characters long and contain only alphanumeric characters or underscores.`,
      );
      return;
    }
    ...
    if (emoji_file.size / 1024 > 256) {
      interaction.reply(
        "File is too large! Please use a file less than 256kb.",
      );
      return;
    }

    const message = await interaction.reply({
      content: `${emoji_name} pending mod approval...`,
      files: [emoji_file],
      fetchReply: true,
    });
