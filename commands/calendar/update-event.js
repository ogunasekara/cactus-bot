  ...
  async execute(interaction) {
    ...
    try {
      ...
      if (existingEvent.createdBy !== interaction.user.id) {
        return await interaction.editReply(
          "❌ You can only update events that you created.",
        );
      }
      ...
      if (startTimeStr) {
        ...
        if (isNaN(startTime.getTime())) {
          return await interaction.editReply(
            "❌ Invalid start time format. Please use YYYY-MM-DD HH:MM",
          );
        }
      ...
      if (endTimeStr) {
        ...
        if (isNaN(endTime.getTime())) {
          return await interaction.editReply(
            "❌ Invalid end time format. Please use YYYY-MM-DD HH:MM",
          );
        }
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          ...
          {
            name: "Start Time",
            value: new Date(updatedEvent.startTime).toLocaleString(),
            inline: true,
          },
      ...
      if (updatedEvent.endTime) {
        embed.addFields({
          name: "End Time",
          value: new Date(updatedEvent.endTime).toLocaleString(),
          inline: true,
        });
      }
