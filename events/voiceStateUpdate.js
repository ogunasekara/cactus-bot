  ...
  async execute(oldState, newState) {
    ...
    if (!oldState.channelId && newState.channelId) {
      const channelName = newState.channel
        ...
        : "Unknown Channel";
      console.log(
        `User ${newState.member.user.username} joined voice channel: ${channelName}`,
      );
      ...
      console.log(
        `Started tracking voice session for ${newState.member.user.username}`,
      );
    }
    ...
    if (oldState.channelId && !newState.channelId) {
      const oldChannelName = oldState.channel
        ...
        : "Unknown Channel";
      console.log(
        `User ${oldState.member.user.username} left voice channel: ${oldChannelName}`,
      );
      ...
      if (activeVoiceSessions.has(userId)) {
        ...
        console.log(
          `User ${oldState.member.user.username} was in voice for ${minutesInChannel} minutes`,
        );
    ...
    if (
      ...
      const newChannelName = newState.channel
        ...
        : "Unknown Channel";
      console.log(
        `User ${newState.member.user.username} moved from ${oldChannelName} to ${newChannelName}`,
      );
