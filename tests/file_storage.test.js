...
describe("FileStorage", () => {
  ...
  beforeEach(async () => {
    testFilePath = path.join(
      TEST_DATA_DIR,
      `test_storage_${Date.now()}_${Math.random().toString(36).substr(2)}.json`,
    );
  ...
  describe("updateEvent", () => {
    ...
    test("should preserve fields not being updated", async () => {
      ...
      const updated = await storage.updateEvent("upd-2", {
        title: "New Title",
      });
    ...
    test("should throw for non-existent event", async () => {
      await expect(storage.updateEvent("nope", { title: "X" })).rejects.toThrow(
        "Event not found",
      );
    });
    ...
    test("should persist update to file", async () => {
      await storage.addEvent(
        sampleEvent({ id: "persist-upd", title: "Before" }),
      );
  ...
  describe("deleteEvent", () => {
    ...
    test("should throw for non-existent event", async () => {
      await expect(storage.deleteEvent("nope")).rejects.toThrow(
        "Event not found",
      );
    });
  });
