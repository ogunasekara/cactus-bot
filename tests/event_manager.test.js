...
describe("EventManager", () => {
  ...
  beforeEach(async () => {
    testFilePath = path.join(
      TEST_DATA_DIR,
      `test_events_${Date.now()}_${Math.random().toString(36).substr(2)}.json`,
    );
  ...
  describe("updateEvent", () => {
    ...
    test("should throw for non-existent event", async () => {
      await expect(
        eventManager.updateEvent("nonexistent", { title: "X" }),
  ...
  describe("deleteEvent", () => {
    ...
    test("should throw for non-existent event", async () => {
      await expect(eventManager.deleteEvent("nonexistent")).rejects.toThrow(
        "Event not found",
      );
    });
  });
