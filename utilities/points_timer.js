...
class PointsTimer {
  ...
  awardPoints() {
    ...
    for (const [userId, session] of this.activeVoiceSessions) {
      ...
      if (!session) {
        console.log(
          `Invalid session data for user ${userId}, removing from tracking`,
        );
        this.activeVoiceSessions.delete(userId);
        continue;
      }

      // Skip users who have hit the daily limit (they'll be retried next tick)
      ...
      if (pointsAwarded > 0) {
        const channelName = session.channelName || "Unknown Channel";
        console.log(
          `Awarded ${pointsAwarded} cactus point(s) to user ${userId} in ${channelName}`,
        );
    ...
    if (this.activeVoiceSessions.size > 0) {
      console.log(
        `Currently tracking ${this.activeVoiceSessions.size} users in voice channels`,
      );
    }
  }
